...
def agregar_datas(df, colun_data, periodo_tempo, colunas_agg, metodos, range_val=None, groupby_cols=None, fusion=False, cont_hora=False):
    ...
    if cont_hora:
        ...
        if periodo_tempo == 'hora':
            ...
            if periodo_tempo == 'semana':
                start_date = dataframe['dia'].max() - pd.DateOffset(days=(range_val - 1) * 7) + pd.DateOffset(weekday=6)
                ...
                start_date = dataframe['dia'].max() - pd.DateOffset(days=(range_val - 1) * 90) + pd.DateOffset(months=(start_date.month - 1) // 3 * 3, day=1)
            elif periodo_tempo == 'semestre':
                start_date = dataframe['dia'].max() - pd.DateOffset(days=(range_val - 1) * 180) + pd.DateOffset(months=(start_date.month - 1) // 6 * 6, day=1)
